...
SELECT ABS(MAX(CASE WHEN 
               		dept.department = 'marketing' THEN emp.salary
               END) - MAX(CASE
...
-- Calcula as médias salarial dos departamentos de vendas e engenharia
select AVG(case when 
           		dept.department = 'sales' then emp.salary
        	end) as  mean_sales, 
       AVG(case when 
           		dept.department = 'engineering' then emp.salary
           end) as mean_engineering
from db_employee emp join db_dept dept
on emp.department_id = dept.id
where dept.department IN ('sales', 'engineering');
